...
bool GamepadController::initialize() {
    ...
    while (timeout > 0) {
        ...
        while (SDL_PollEvent(&event)) {
            if (event.type == SDL_EVENT_GAMEPAD_ADDED) {
                ...
                if (gamepad_) {
                    std::cout << "手柄已连接: " << SDL_GetGamepadName(gamepad_) << std::endl;
                    return true;
                }
            }
        }
    ...
    std::cout << "5秒内未检测到手柄，请确保手柄已连接并被系统识别" << std::endl;
    return false;
}
    ...
    while (SDL_PollEvent(&event)) {
        switch (event.type) {
            ...
            case SDL_EVENT_GAMEPAD_REMOVED:
                if (gamepad_ && event.gdevice.which == SDL_GetGamepadInstanceID(gamepad_)) {
                    SDL_CloseGamepad(gamepad_);
                    gamepad_ = nullptr;
                    std::cout << "手柄断开连接" << std::endl;
                }
                break;
            ...
            case SDL_EVENT_GAMEPAD_BUTTON_UP:
                if (button_callback_ && gamepad_ && 
                    event.gbutton.which == SDL_GetGamepadInstanceID(gamepad_)) {
            ...
            case SDL_EVENT_GAMEPAD_AXIS_MOTION:
                if (axis_callback_ && gamepad_ && 
                    event.gaxis.which == SDL_GetGamepadInstanceID(gamepad_)) {
...
void GamepadController::updateState() {
    ...
    // 读取按钮
    current_state_.button_a = SDL_GetGamepadButton(gamepad_, SDL_GAMEPAD_BUTTON_A);
    current_state_.button_b = SDL_GetGamepadButton(gamepad_, SDL_GAMEPAD_BUTTON_B);
    current_state_.button_x = SDL_GetGamepadButton(gamepad_, SDL_GAMEPAD_BUTTON_X);
    current_state_.button_y = SDL_GetGamepadButton(gamepad_, SDL_GAMEPAD_BUTTON_Y);
